...
# --- 2. 【Zenn記事準拠】最上層(window.top)を日本語化し、アイコン文字化けを物理的に防ぐ ---
# window.top を使うことで Streamlit Cloud の英語設定を「日本語」に上書きします
components.html("""
    <script>
...
for i, (name, rent, s_price) in enumerate(patterns):
    ...
    with res_cols[i]:
        st.markdown(f'<div class="detail-card"><div style="font-size:0.75rem;color:#94a3b8;font-weight:bold;margin-bottom:5px;">{name}</div><div class="detail-item">販売: <span class="detail-val-text">{int(s_price):,}</span>万</div><div class="detail-item">CF: <span class="detail-val-text">{int(net_rent - pay):,}</span>円/月</div></div>', unsafe_allow_html=True)