...
# --- 2. 【Zenn記事準拠】最上層(window.top)を日本語化し、翻訳バグを物理的に防ぐ ---
components.html("""
    <script>
        const topDoc = window.top.document;
        
        // 1. ページ全体の言語を日本語に固定（翻訳ポップアップの根本を止める）
        ...
        // 3. 【アイコン文字化け対策】最上層に直接スタイルを注入
        // アイコン用のテキスト(Ligature)を消し、矢印を画像（SVG）として表示させる
...
st.markdown("""
    ...
    * { 
    ...
    /* ★ 会社総粗利（青枠ハイライト）の復活 */
    .total-profit-card { border: 2px solid #3b82f6 !important; background-color: #f0f7ff !important; }
...
with s1:
    st.markdown(f'<div class="metric-card notranslate"><div class="metric-label">仕入粗利</div><div class="metric-value">{prof_a:.1f}万</div><div class="rate-text" style="color:#64748b;">{rate_a:.2f}%</div></div>', unsafe_allow_html=True)
with s2:
    st.markdown(f'<div class="metric-card notranslate"><div class="metric-label">VU粗利</div><div class="metric-value">{prof_b:.1f}万</div><div class="rate-text" style="color:#64748b; font-size:0.75rem;">工事費込</div></div>', unsafe_allow_html=True)
with s3:
    st.markdown(f'<div class="metric-card total-profit-card notranslate"><div class="metric-label">会社総粗利</div><div class="metric-value">{total_p:.1f}万</div><div class="rate-text">{total_r:.2f}%</div></div>', unsafe_allow_html=True)
...
for i, (name, rent, s_price) in enumerate(patterns):
    ...
    with res_cols[i]:
        st.markdown(f'<div class="detail-card notranslate"><div style="font-size:0.75rem;color:#94a3b8;font-weight:bold;margin-bottom:5px;">{name}</div><div class="detail-item">販売: <span class="detail-val-text">{int(s_price):,}</span>万</div><div class="detail-item">CF: <span class="detail-val-text">{int(net_rent - pay):,}</span>円/月</div></div>', unsafe_allow_html=True)